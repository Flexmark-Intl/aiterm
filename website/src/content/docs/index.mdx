...
hero:
  ...
  actions:
    - text: Get Started
      link: /aiterm/guides/getting-started/
      icon: right-arrow
      variant: primary
    - text: View on GitHub
      link: https://github.com/Flexmark-Intl/aiterm
      icon: external
---
...
<CardGrid stagger>
  ...
  <Card title="Claude Code Integration" icon="puzzle">
    30+ MCP tools let Claude Code navigate your workspaces, manage notes, control preferences, review diffs, and discover what's running in your tabs — all without leaving the terminal. Auto-capture session IDs and auto-resume too.
  </Card>
  <Card title="Built-in Editor" icon="document">
    Click any file path in terminal output to open it in an editor tab. Works over SSH too — remote files load transparently via SCP. 50+ languages, auto-reload on external changes, and conflict detection when files are dirty.
  </Card>
  <Card title="Smart Triggers" icon="setting">
    Regex triggers watch terminal output for patterns and fire actions — notifications, commands, variable capture. Scope triggers to specific tabs or fire them globally. 6+ built-in Claude Code triggers included.
  </Card>
  ...
  <Card title="GPU-Accelerated Terminal" icon="laptop">
    WebGL rendering for buttery-smooth output. Full terminal state survives restarts — tab layout, workspace state, scrollback history all come back after a reboot.
  </Card>
